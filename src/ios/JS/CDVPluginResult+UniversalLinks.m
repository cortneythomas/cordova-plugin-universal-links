...
@implementation CDVPluginResult (UniversalLinks)
 ...
 */
+ (NSDictionary *)prepareMessageForHost:(Host *)host originalURL:(NSURL *)originalURL {
 ...
 *  @return event name
 */
+ (NSString *)getEventNameBasedOnHost:(Host *)host originalURLComponents:(NSURLComponents *)urlComponents {
    NSString *eventName = host.event;
    NSArray<Path *> *hostPaths = host.paths;
    ...
    for (Path *hostPath in hostPaths) {
